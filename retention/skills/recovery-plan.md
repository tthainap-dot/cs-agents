...
⚠️ **Pré-requisito obrigatório, verificar antes de usar esta tática:** conferir `knowledge/churn-taxonomy.md` ("Fatores externos" → "Uso da ferramenta não é obrigatório") — **empresas com menos de 20 funcionários não têm obrigatoriedade legal de controle de jornada no Brasil**. Para essas contas, o argumento de risco legal abaixo é **factualmente falso** e não deve ser usado — apresentar um risco que não existe violaria a regra de nunca inventar informação (`common/standards/anti_hallucination.md`). Confirmar o headcount da conta antes de aplicar esta tática.

Quando a causa do cancelamento é **abandonar uma solução que resolve uma dor de compliance/regulatória** por uma alternativa manual (ex.: voltar para planilha, processo manual, método informal), **e a conta tem 20+ funcionários**, considerar o risco legal/financeiro que o cliente reintroduz como um ângulo de recuperação válido — mesmo em casos que pareçam "não recuperáveis" à primeira vista.

Isso não é pressão de venda — é informação genuína de risco que o cliente pode não ter considerado: erro de registro, exposição a multa, fragilidade em auditoria, risco de processo trabalhista. Essas obrigações legais se aplicam a partir do limiar de 20 funcionários, independentemente de a empresa estar com o processo automatizado ou não.
