...
⚠️ **Pré-requisito obrigatório, verificar antes de usar esta tática:** este argumento só é válido se a obrigação regulatória/legal realmente se aplicar à conta em questão. **Nunca presumir que a obrigação é universal** — verificar em `knowledge/` (ex.: `churn-taxonomy.md`, ou o arquivo de conhecimento regulatório específico da empresa) se existe algum limiar, exceção ou condição (ex.: porte da empresa, setor, região, tipo de contrato) abaixo/fora do qual a obrigação não se aplica. Se a conta cair nessa exceção, este argumento é **factualmente falso** para ela e não deve ser usado — apresentar um risco que não existe violaria a regra de nunca inventar informação (`common/standards/anti_hallucination.md`).

Quando a causa do cancelamento é **abandonar uma solução que resolve uma dor de compliance/regulatória** por uma alternativa manual (ex.: voltar para planilha, processo manual, método informal), **e a obrigação regulatória de fato se aplica à conta** (conforme verificado acima), considerar o risco legal/financeiro que o cliente reintroduz como um ângulo de recuperação válido — mesmo em casos que pareçam "não recuperáveis" à primeira vista.

Isso não é pressão de venda — é informação genuína de risco que o cliente pode não ter considerado: erro de registro, exposição a multa/penalidade, fragilidade em auditoria, risco de processo/disputa. A natureza exata do risco (qual lei, qual multa, qual limiar) é sempre específica do domínio regulatório e do país da empresa — nunca inventar o detalhe, buscar sempre em `knowledge/`.
